...
La presente aplicación corresponde a la **Semana 12** de la asignatura Programación Orientada a Objetos. Es la evolución del proyecto de la Semana 11 y conserva el sistema de ventas relacional entre usuarios y productos, el control de stock y la persistencia JSON de todas las colecciones principales.
...
## Mejoras de colecciones de la Semana 12

Se conservaron las listas principales `_productos`, `_usuarios` y `_ventas` para almacenar objetos, recorrerlos, listarlos y persistirlos en JSON. Dentro de `servicios/restaurante.py` se agregaron estructuras auxiliares para evitar recorridos completos en las operaciones frecuentes:

- `_productos_por_codigo`: `dict[str, Producto]` indexado por código normalizado. Se utiliza en la búsqueda y en la validación de códigos duplicados.
- `_usuarios_por_identificacion`: `dict[str, Usuario]` indexado por identificación normalizada. Se utiliza en la búsqueda y en la validación de identificaciones duplicadas.
- `_ventas_por_usuario`: `dict[str, list[Venta]]` que agrupa ventas por identificación de usuario. Se utiliza en `consultar_ventas_usuario`.
- `_categorias`: `set[str]` para mantener categorías únicas y consultar pertenencia sin duplicados.

Los índices se reconstruyen al cargar cada lista desde JSON y se actualizan al registrar, modificar, vender o eliminar información. Las claves se normalizan con `casefold()`, por lo que las búsquedas mantienen el comportamiento sin distinguir mayúsculas y minúsculas.
...
### Prueba 9: Índices en memoria
- ✓ La búsqueda de productos usa el índice por código.
- ✓ La búsqueda de usuarios usa el índice por identificación.
- ✓ La consulta de ventas usa el índice agrupado por usuario.
- ✓ Los índices se actualizan después de registrar, modificar, eliminar y vender.
- ✓ Los índices se reconstruyen después de cargar nuevamente los archivos JSON.

## Notas de Implementación

1. **Colecciones principales e índices**: Se mantienen listas de objetos `Producto`, `Usuario` y `Venta` para almacenar, recorrer y persistir. Los diccionarios auxiliares se usan para búsquedas por código o identificación y para agrupar ventas por usuario.
...
## Evolución de la Semana 12 respecto a la Semana 11

| Aspecto | Semana 11 | Semana 12 |
|--------|-----------|----------|
| Listas principales para persistencia | **Sí** | **Sí** |
| Búsqueda de productos por código | Recorrido de lista | **Índice `dict`** |
| Búsqueda de usuarios por identificación | Recorrido de lista | **Índice `dict`** |
| Consulta de ventas por usuario | Filtrado de toda la lista | **Índice `dict` de listas** |
| Categorías únicas | `set` creado al consultar | **`set` mantenido en memoria** |
| Reconstrucción de auxiliares al iniciar | No aplica | **Sí, desde JSON** |

## Autor
Cristhian Tenesaca - POO Semana 12
